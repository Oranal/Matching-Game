    ...
    try:
        ...
        if request.POST:
            ...
            if {'institution': institution_} in Account.objects.values('institution').distinct():
                ...
                django.setup()
                try:
                    usr = Account.objects.get_or_create(
                        first_name=firstname_, last_name=lastname_, username=username_, password=password_, institution=institution_, role=form.get_role())[0]
                    usr.save()
                except:
    ...
    try:
        ...
        if request.POST:
            ...
            if my_bag.get('user')['role'] == 'Kindergarden':
                ...
                all_categories = my_bag.get('teacher')['categories']
            ...
            django.setup()
            try:
                usr = Account.objects.get_or_create(categories=categories_, first_name=firstname_, last_name=lastname_, username=username_,
        ...
        if request.GET:
            ...
            for teacher_details in Account.objects.values():
                if teacher_details['username'] == username_:
                    my_bag.set('teacher', teacher_details)
                    my_bag.set('institution', my_bag.get(
                        'teacher')['institution'])
                    break
    ...
    try:
        if request.POST:
            ...
            os.environ.setdefault('DJANGO_SETTINGS_MODULE',
                                  'matching_game.settings')
            django.setup()
            game = Board.objects.get_or_create(category=my_bag.get(
                'category')[0], data=input_json_format_converter(my_bag.get('category')[1]))[0]
            game.save()
            ...
            user.categories = updated_score_for_category(my_bag.get(
                'user')['categories'], my_bag.get('game')['category'], int(request.GET['score']))
            user.rating = user.rating+int(request.GET['score'])
            user.save()
            admin_user = Account.objects.get(role="Administrator")
            admin_user.categories = updated_score_for_category(
                admin_user.categories, my_bag.get('game')["category"], 1)
            admin_user.save()
...
def game_topic(request):
    try:
        if request.GET:
            ...
            my_bag.get('category')[1][my_bag.get(
                'topic')].append(request.POST['data1'])
            my_bag.get('category')[1][my_bag.get(
                'topic')].append(request.POST['data2'])
            form = forms.SingleForm()
...
def extra_card(request):
    try:
        if request.POST:
            my_bag.get('category')[1][my_bag.get(
                'topic')].append(request.POST['data'])
            form = forms.SingleForm()
            return render(request, 'accounts/game_topic.html', {'massage': 'extra', 'form': form, 'user': my_bag.get('user')})
        return render(request, 'accounts/game_info.html', {'massage': left, 'form': form, 'user': my_bag.get('user')})
    except:
        ...
        if request.GET:
            if 'add' in request.GET:
                if request.GET['game'] not in my_bag.get('user')['categories'].values():
                    ...
                    user.categories = jasonedCategories(my_bag.get(
        ...
        if request.GET:
            ...
            for key in my_bag.get('game')['data'].keys():
                ...
                for card in my_bag.get('game')['data'][key]:
                    card_data[key].append(
                        my_bag.get('game')['data'][key][card])
...
def score_board(request):
    try:
        if my_bag.get('user')['role'] == 'Kindergarden':
            ...
            return render(request, 'accounts/score_board.html', {'user': my_bag.get('user'), 'users': listed_players(Account.objects.all().filter(role="Child").values('first_name', 'last_name', 'rating', 'categories'))})
    except:
        return render(request, 'accounts/games.html', {'user': {'role': 'error'}})
...
def search(request):
    try:
        if request.GET:
            ...
            if len(name) == 1:
                if my_bag.get('user')['role'] == 'Kindergarden':
                    users.append(Account.objects.filter(role='Child', first_name=name[0], institution=my_bag.get(
                        'user')['institution']).values('first_name', 'last_name', 'rating', 'categories'))
                    users.append(Account.objects.filter(role='Child', last_name=name[0], institution=my_bag.get(
                        'user')['institution']).values('first_name', 'last_name', 'rating', 'categories'))
                else:
                    users.append(Account.objects.filter(role='Child', first_name=name[0]).values(
                        'first_name', 'last_name', 'rating', 'categories'))
                    users.append(Account.objects.filter(role='Child', last_name=name[0]).values(
                        'first_name', 'last_name', 'rating', 'categories'))
            else:
                for i in range(len(name) + 1):
                    ...
                    if last == "":
                        if my_bag.get('user')['role'] == 'Kindergarden':
                            users.append(Account.objects.filter(role='Child', first_name=first, institution=my_bag.get(
                                'user')['institution']).values('first_name', 'last_name', 'rating', 'categories'))
                        else:
                            users.append(Account.objects.filter(role='Child', first_name=first).values(
                                'first_name', 'last_name', 'rating', 'categories'))
                    elif first == "":
                        if my_bag.get('user')['role'] == 'Kindergarden':
                            users.append(Account.objects.filter(role='Child', last_name=last, institution=my_bag.get(
                                'user')['institution']).values('first_name', 'last_name', 'rating', 'categories'))
                        else:
                            users.append(Account.objects.filter(role='Child', last_name=last).values(
                                'first_name', 'last_name', 'rating', 'categories'))

                    else:
                        if my_bag.get('user')['role'] == 'Kindergarden':
                            users.append(Account.objects.filter(role='Child', first_name=first, last_name=last, institution=my_bag.get(
                                'user')['institution']).values('first_name', 'last_name', 'rating', 'categories'))
                        else:
                            users.append(Account.objects.filter(role='Child', first_name=first, last_name=last).values(
                                'first_name', 'last_name', 'rating', 'categories'))
...
def listed_players(data):
    ...
    for user in data:
        each_user = []
        each_user.append([user['first_name'] + " " +
                         user['last_name'], user['rating']])
